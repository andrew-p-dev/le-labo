<template>
  <div>
    <v-row class="paper-card black--text unselectable" no-gutters>
      ...
      <v-col cols="330px" v-if="notes">
        
      </v-col>
    </v-row>
    <div :style="`width: 330px${ $vuetify.breakpoint.smAndDown ? ';margin-left: calc((100vw - 330px) / 3)' : ''}`" class="pa-1 pt-3 d-flex justify-end" @click="showDescription = !showDescription">
      <v-btn class="black--text" text v-if="!showDescription">Read more</v-btn>
      <div v-if="showDescription" transition="expand-transition">
